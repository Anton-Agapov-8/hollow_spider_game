        ...
        if breakReason == 1:
            ...
            if not seePlayer:
                ...
                if rv == 1:
                    ...
                    self.creatureAngle = (self.creatureAngle - numpy.pi / 4)
        listx = list(range(int(playerX) - 1 * (k // 5), int(playerX) + 1 * (k // 5)))
        listy = list(range(int(playerY) - 1 * (k // 5), int(playerY) + 1 * (k // 5)))
...
def place_sprites(level_map, mapX, mapY, color1, color1_2, color2, color3, scale_koeff1, scale_koeff2, scale_koeff3,
                  ...
                  steps_frame_number_spider, text_list, button_number, give_gun, weapon_list, gun=None):
    sprites = []
    for enemyX in range(mapX):
        for enemyY in range(mapY):
            ...
            if level_map[enemyX, enemyY] == color1_2:
                sprites.append(
                    Creature(enemyX, enemyY, 0, deg2rad(30), 20, 100, 20, scale_koeff1, sprite_sheet_wasp2,
                             dead_sprite_wasp2,
                             steps_frame_number_wasp))
...
def main(file, k, playerAngle, weapon_list, gun, laser, text, buttun_number):
    pygame.init()
    ...
    enemies = place_sprites(level_map, mapX, mapY, (255, 216, 0, 255), (255, 216, 106, 255), (0, 255, 0, 255),
                            ...
                            buttun_number, True, weapon_list, gun)
            ...
            if angle_difference > (11 * numpy.pi / 6) or angle_difference < (numpy.pi / 4):
                ...
                for i in range(int(distanceToSprite / Raykoeff)):
                    ...
                    if type(enemies[en]) in [type(comparizon_creature), type(comparizon_creature2)]:
                        ...
                        if level_map[RayX, RayY] == (0, 0, 0, 255) or level_map[RayX, RayY] == (128, 128, 128, 255):
                            enemies[en].set_distanceToPlayer(999)
            else:
                enemies[en].set_distanceToPlayer(999)
            if type(enemies[en]) in [type(comparizon_creature), type(comparizon_creature2)]:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and weapon.get_can_shoot():
                ...
                for en in range(len(enemies)):
                    if type(enemies[en]) in [type(comparizon_creature), type(comparizon_creature2)]:
                        ...
                        for xx in range(-k // 5, k // 5):
                            for yy in range(-k // 5, k // 5):
...
if __name__ == '__main__':
    ...
    result, surf, weapon_list = main(file, k, deg2rad(270), weapon_list, gun, laser, ['Spider', 'Wasp'], 100)
    running = True
